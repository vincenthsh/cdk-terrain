...
export async function convert({
  ...
  try {
    const { all, stats } = await hcl2cdkConvert(input, {
      ...
      codeContainer: stack ? "cdktn.TerraformStack" : "constructs.Construct",
    });
    output = all;
...
export async function login(argv: { tfeHostname: string }) {
  ...
  async function showUserDetails(authToken: string) {
    ...
    if (userAccount) {
      ...
      console.log(
        chalkColour`\n{greenBright cdktn has successfully configured Terraform Cloud credentials!}`,
      );
...
export async function debug(argv: any) {
  ...
  if (argv.json) {
    ...
    console.log(chalkColour`{bold {greenBright cdktn debug}}`);
  ...
  if (constraintsToUpdate.length > 0) {
    ...
    console.log(
      `${constraintsToUpdate.length} local provider${
        singular ? " has" : "s have"
      } been updated. Running cdktn get to update...`,
    );
