...
export const providerVersionMismatch = () =>
  new Error(
    `Version mismatch detected: Your provider bindings are built for an older version of CDKTn.
Upgrade your pre-built provider or re-run cdktn get with a more recent version (>= 0.10) of the cdktn-cli.
    `,
  );
...
Glob characters are not supported in stack names because we use them to deploy and destroy multiple stacks. For example, 'cdktn deploy \`*-production\`' deploys all stacks with names that end in '-production'.
`);
...
Testing.synth returns the JSON representation of your stack. CDKTN can use assertions with this JSON to check the composition of your stacks, like so: "Testing.toHaveResource(Testing.synth('TerraformStack instance'), S3Bucket)"
...
export const cannotConcatenateStringsInTokenizedStringArray = (
  ...
  new Error(
    ...
    You are likely mutating the value inside your array, which CDKTN cannot resolve into the appropriate runtime value.`,
  );
...
export const cannotConcatenateStringsInTokenizedMap = (tokenizedMap: any) =>
  new Error(
    ...
    This can happen if you mutate the key of the map. Do not mutate tokenized Maps, because it makes it impossible for CDKTN to translate them.`,
  );
...
export const canOnlyEncodePositiveIntegers = () =>
  new Error(
    `Can only encode positive integers into Tokens. This is a bug in CDKTN, please file a bug report.`,
  );
...
export const indexTooLargeToEncode = (index: number) =>
  new Error(
    ...
    This happens if you have too many Tokens in a single CDKTN application. This can also occur if you are accidentally creating numeric tokens in an infinite loop. For example, calling 'Token.asNumber()' too many times.
...
const LIST_ERROR_EXPLANATION = `In CDKTN, we represent lists, with values unknown until after runtime, as arrays with a single element— a string token (["Token.1"]).


We do this because CDKTN does not know the length of the list at compile time, meaning CDKTN has yet to invoke Terraform to communicate with the cloud provider. CDKTN later invokes Terraform on the synthesized static JSON file.
...
const unknownTokenExplanation = `This means you are trying to access a Token value that does not exist. This can only happen if the Token is from another CDK (i.e., AWS CDK, CDK8s, etc.) and is unknown to this CDKTN Application. You either need to make the other CDK resolve this Token before CDKTN tries to resolve it, or work around using this Token at all.
